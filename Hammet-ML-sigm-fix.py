...
def Hammett_data(data):
    ...
    #dicnewrho, newrhos,  E0 = fix_param(data - x0, sigmas, dicrho)
    #E0 = E0 + x0
    return(dicrho, x0, dicsigmas, rhos, sigmas)
...
def Hamm_param2(traindf, testdf):
    ''' This function predicts the Activation Energy using the Hammett regression
    ...
    '''
    dicrho, x0, dicsigmas, rhos, sigmas = Hammett_data(reshapecopy(traindf, "Barrier"))
    dicsigmas_test = calc_sigmas(reshapecopy(testdf, "Barrier") - x0,rhos)[0]
    dicsigmfin = {**dicsigmas_test, **dicsigmas}
    #dicnewrho, newrhos,  E0 = fix_param(reshapecopy(traindf, "Barrier") - x0, sigmas, dicrho)
    #E0 = E0 + x0
    dicE0 = dict(zip(combinations, x0))
...
def Hamm_param(traindf, testdf):
    ...
    return(dicnewrho, dicsigmfin, dicE0)
...
def fix_param(deltadata, sigmas, dicrhos):
    ...
    return(dicnewrho, newrho, newEa0)
...
if __name__ == '__main__':
    # CLI arguments
    parser = argparse.ArgumentParser(description='Hammet and ML')
    parser.add_argument('--hammett', action='store_true', help='Calculate Hammett')
    parser.add_argument('--ml', action='store_true', help='Calculate ML model')
    parser.add_argument('--plot', action='store_true', help='Plot learning curve')
...
plt.plot(np.arange(200,1301,100), np.array(er_dml_ls), '--o', label=r'$\Delta$-ML')
plt.plot(np.arange(200,1301,100), np.array(er_ml_ls), '--o', label=r'ML')
#plt.plot(np.arange(100,800,100), np.array(er_ml1), '--o', label=r'ML1')
plt.legend()
plt.xscale('log')
plt.yscale('log')
plt.ylim([0,8])
...
plt.xlabel('Training set size')
plt.ylabel('MAE (kcal/mol)')
plt.title(r'All $\sigma$ calculated with the same set of $\rho$ and E0')
plt.grid(True)
plt.show()
#plt.savefig("ugly-learning-curves.png", dpi=500, format="png")
...
for TSsize in np.arange(200,1301,100):
    ...
    for fold in range(15):
        ...
        dicrho, dicsigmas, dicE0 = Hamm_param2(traindf, testdf)
    ...
    #er_ml1.append(np.mean(np.array(ML_err)))
    er_hamm_ls2.append(np.mean(np.array(hammae)))
    er_dml_ls2.append(np.mean(np.array(dmlmae)))
    er_ml_ls2.append(np.mean(np.array(mlmae)))
...
get_ipython().run_line_magic('matplotlib', 'notebook')
plt.plot(np.arange(200,1301,100), np.array(er_hamm_ls2), '--o', label='Hammett')
plt.plot(np.arange(200,1301,100), np.array(er_dml_ls2), '--o', label=r'$\Delta$-ML')
plt.plot(np.arange(200,1301,100), np.array(er_ml_ls2), '--o', label=r'ML')
#plt.plot(np.arange(100,800,100), np.array(er_ml1), '--o', label=r'ML1')
plt.legend()
plt.xscale('log')
plt.yscale('log')
...
plt.xlabel('Training set size')
plt.ylabel('MAE (kcal/mol)')
plt.title(r'No additional fixing of $\rho$ and E0')
plt.grid(True)
plt.show()
#plt.savefig("ugly-learning-curves.png", dpi=500, format="png")
